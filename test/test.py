...
def get_test_info(test, nvim, vimrc):
    ...
    if desc[0]:
        ...
        return "./test.py %s %s\tusing %s vimrc" % (
            ("", "-n ")[nvim], test, rc)
...
def get_paths(test, f):
    ...
    paths["in_file"] = Path(
        'tests/', test, 'input_file.txt').resolve(strict=True)
    paths["exp_out_file"] = Path(
        'tests/', test, 'expected_output_file.txt').resolve(strict=True)
    paths["gen_out_file"] = Path(
        'tests/', test, 'generated_output_file.txt').resolve()
...
def run_core(paths, nvim=False):
    ...
    if nvim:
        ...
        server = multiprocessing.Process(
            ...
            args=(VIM + " -u " + str(paths["vimrc"]) +
                  ' --listen ' + str(paths["socket"]),),
            kwargs={'shell': True}
        )
        server.start()
        time.sleep(1)
        ...
        vim = vimrunner.Server(
            noplugin=False, vimrc=paths["vimrc"], executable=VIM)
        client = vim.start()
...
def main():
    ...
    parser.add_argument('test', nargs='?',
                        help='run <test> only instead of running all tests')
    parser.add_argument('-n', '--nvim', action='store_true',
                        help='run in neovim instead of vim')
    ...
    if args.list:
        ...
        print_banner("Starting yanktools.vim tests", f)
